...
public class QuadTreeViewer extends JPanel implements MouseListener,
		...
		if (tree.isLeaf) {
			...
			synchronized (tree.points) {
					for (Point2D p : tree.points) {
						...
						g.drawOval((int)(p.x-pointBorderSize), (int)(p.y-pointBorderSize), 2*pointBorderSize, 2*pointBorderSize);
					}
				}
			}
		} else {
	...
	public static void main(String input[]) throws InterruptedException {
		...
		QuadTreeViewer viewer = new QuadTreeViewer();
		viewer.pointSize = 3;
		viewer.pointBorderSize = border;
