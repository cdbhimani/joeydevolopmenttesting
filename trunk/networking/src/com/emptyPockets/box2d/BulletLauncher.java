...
public class BulletLauncher {
	...
	public BulletLauncher(Body owner, Vector2 pos, Vector2 dir, float speed, long shootInterval){
		setOwner(owner);
		setPos(pos);
		setDir(dir);
		setSpeed(speed);
	...
	private boolean createBullet(){
		updateBodyDef();
		final Body newBullet = owner.getWorld().createBody(bodyDef);
		CircleShape shape = new CircleShape();
		shape.setRadius(bulletRadius);
		newBullet.createFixture(shape, bulletDensity);
		newBullet.setUserData(this);
		shape.dispose();
		return true;
	}
